          ...
          <div className="card">
            ...
            <div className="field">
              ...
              <p className="field-hint">
                Setting this shows you what is already there as you browse.
              </p>
            </div>
            ...
            {pickedList.length === 0 && manual.length === 0 ? (
              <p className="field-hint">Nothing selected yet.</p>
            ) : (
              <div className="item-list">
                {pickedList.map(([variantId, v]) => (
                  <div key={variantId} className="item-line">
                    <span className="item-qty">{v.qty}</span>
                    <span style={{ flex: 1 }}>
                      <span className="item-name">{v.name}</span>
                      <span className="cell-sub">{v.sku || 'No SKU'}</span>
                    </span>
                    <button className="linklike" onClick={() => removePicked(variantId)}>
                      Remove
                    </button>
                  </div>
                ))}
                {manual.filter((m) => m.name.trim()).map((m, i) => (
                  <div key={`m-${i}`} className="item-line">
                    <span className="item-qty">{m.qty}</span>
                    <span>
                      <span className="item-name">{m.name}</span>
                      <span className="cell-sub">Added by hand</span>
                    </span>
                  </div>
                ))}
              </div>
